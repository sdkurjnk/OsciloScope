...
**검사에 실패(✕)한 도구는 선택할 수 없다.** 고친 뒤 다시 검사해서 통과하면 바로 풀린다.
경고(!)는 실행을 막지 않는다.
...
- **9번은 model을 한 번만 만들고 `render`만 2회 반복한다.** 그래서 `analyze`가 비결정적이어도
  9번은 걸리지 않고 7번이 잡는다. 대신 `render`가 model을 수정하면 9번에 걸린다.
