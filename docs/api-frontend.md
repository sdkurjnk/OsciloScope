...
검사에서 `fail`이 난 도구는 로드에 실패한 도구와 같이 취급한다 — 선택 불가, 사유 표시,
이미 선택돼 있었다면 해제. `TOOL_ERROR`로 실행 중 터진 도구도 같다. 검사가 결과 아이콘만
그리고 실행을 막지 않으면 `fail`을 보고도 그대로 렌더할 수 있어 검사를 신뢰할 수 없게 된다.

검사 버튼은 남겨 둔다. 고친 뒤 다시 검사해서 통과하면 바로 복구된다.

> **미검사(아이콘 없음) 상태는 막지 않는다.** 검사 결과가 세션 메모리(`state.checks`)에만
> 있어서, 사이드바를 접었다 펴면 전부 미검사로 돌아가 START가 잠긴다. 기본 도구도 처음에는
> 미검사라 첫 실행 자체가 불가능해진다. 확장 쪽에 `mtime`과 함께 결과를 영속화한 뒤에
> 게이트를 넓히는 것이 맞다.
